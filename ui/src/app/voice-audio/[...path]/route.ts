...
async function proxyMediaRequest(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        detail: `Media request failed while proxying to ${backendUrl}: ${message}`,
      },
      { status: 502 },
    );
  }
...
export const OPTIONS = proxyMediaRequest;
